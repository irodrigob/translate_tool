...
public section.
  ...
  methods GET_ALLOWED_OBJECTS
    returning
      value(R_OBJECTS) type TR_OBJECT_TEXTS .
  methods GET_TLANG
    returning
      value(R_TLANG) type LXE_TT_LXEISOLANG .
...
*"* do not include other source files here!!!
  ...
  TYPES:
    ty_t_main_fields TYPE STANDARD TABLE OF ts_main_fields .
  ...
  DATA it_object_list TYPE zcl_translate_cmp=>tt_object_list .
  DATA it_lxe_list TYPE zcl_translate_lxe=>tt_lxe_list .
...
* en otros puntos del programa
  ...
  IF sy-subrc = 0.
    r_exist = <ls_object_list>-ref_class->check_object_exists( iv_pgmid = <ls_object_list>-pgmid
                                                               iv_object   = i_object
...
* Obtengo los objetos que: 1) pueden ser traducidos 2) podemos obtener componentes.
  zcl_translate_cmp=>get_objectlist( IMPORTING et_object_list = it_object_list ).

* Obtengo los objetos cuyo proceso de traduccion esta implementado.
  zcl_translate_lxe=>get_lxelist( IMPORTING et_lxe_list = it_lxe_list  ).
...
* Paso los parámetros para condicionar la búsqueda.
    <ls_object_list>-ref_class->set_params( EXPORTING iv_depth_refs = d_depth_refs ).
